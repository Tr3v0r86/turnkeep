 ...
 */
/* #3 fix: CLIB malloc (host heap) — no fixed pool, so large layer buffers allocate fine.
 * Device analog = CONFIG_LV_USE_CLIB_MALLOC=y, where big allocs land in PSRAM. */
...
#define LV_STDARG_INCLUDE       <stdarg.h>
     ...
     * Dead now (LV_USE_STDLIB_MALLOC = CLIB above). Was 64K and matched the device's old
     * builtin pool — that 64K ceiling was the focus-freeze (#3); CLIB removes it. */
...
#define LV_USE_SNAPSHOT 1   /* sim: SIM_SHOT=out.ppm headless screenshot (sim_main.c) */
